...
👉 배포 링크 - https://menu-mate-puce.vercel.app
...
일상에서 "오늘 뭐 먹지?"라는 고민을 하는 경우가 자주 발생하지만, 원하는 메뉴를 떠올리거나 레시피를 찾는 과정은 번거로운 경우가 많습니다.
...
식약처에서 제공하는 레시피 Open API를 활용하여 실제 데이터를 기반으로 서비스를 구성했습니다.  
...
- **Next.js (App Router)**
  - 파일 기반 라우팅과 서버 컴포넌트와 클라이언트 컴포넌트를 혼합하며 활용해 페이지 구조를 구성했습니다.
...
- **TanStack Query**
  ...
  - loading / error 상태를 일관되게 관리하여 UI 복잡도를 줄였습니다.
...
프로젝트 루트 경로에 '.env.local'파일을 생성한 뒤, 아래 환경 변수를 추가합니다.
...
프로덕션 환경과 유사한 조건에서 프로젝트를 확인할 수 있습니다.
