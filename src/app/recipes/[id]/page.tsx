...
const SECTION_DESCRIPTION_STYLE = 'mt-1 text-[15px] leading-6 text-[var(--mm-muted)]';

const INGREDIENT_ITEM_STYLE =
  'flex items-start gap-3 rounded-2xl bg-[var(--mm-inner-card-soft)] px-4 py-3';

const STEP_ITEM_STYLE = 'flex flex-col gap-3 rounded-2xl bg-[var(--mm-inner-card-soft)] px-4 py-4';

const STEP_LABEL_STYLE =
  'w-fit rounded-full bg-[var(--mm-primary-soft)] px-3 py-1 text-[14px] font-semibold text-[var(--mm-primary-hover)]';
      ...
      <article className={ARTICLE_STYLE}>
        <header className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1.5">
            <span className="w-fit rounded-full bg-[var(--mm-primary-soft)] px-3 py-1 text-[13px] font-semibold text-[var(--mm-primary-hover)]">
              Recipe
            </span>

            <h1 className="text-2xl font-bold tracking-tight text-[var(--mm-text)] md:text-3xl">
              {recipe.name}
            </h1>

            <p className="text-[15px] leading-6 text-[var(--mm-muted)]">
            ...
            <ul className="grid gap-3">
              {ingredientList.map((ingredient, index) => (
                <li key={`${recipe.id}-ingredient-${index}`} className={INGREDIENT_ITEM_STYLE}>
                  ...
                  <span className="text-[15px] leading-6 text-[var(--mm-text)]">{ingredient}</span>
                </li>
              ))}
            </ul>
          ) : (
            ...
            <ol className="flex flex-col gap-4">
              {recipe.steps.map((step, index) => (
                <li key={`${recipe.id}-step-${index}`} className={STEP_ITEM_STYLE}>
                  <span className={STEP_LABEL_STYLE}>{index + 1}단계</span>

                  <p className="text-[15px] leading-7 text-[var(--mm-text)]">{step.description}</p>
